<?php
	...
	function hasUploadedFile($fileArray)
	{
		// Kiểm tra xem mảng file có tồn tại không
		if (empty($fileArray['error'])) {
			return false;
		}
	
		// Lặp qua tất cả các phần tử trong mảng error
		foreach ($fileArray['error'] as $key => $error) {
			if ($error['taptin'] === 0) {
				// Nếu tìm thấy bất kỳ file nào có mã lỗi 0,
				// có nghĩa là có ít nhất một file đã được tải lên.
				return true;
			}
		}
	
		// Nếu không tìm thấy file nào có mã lỗi 0,
		// tức là không có file nào được tải lên thành công.
		return false;
	}
	...
	function save_photo()
	{
		...
		if($id)
		{
			...
			{
				$file_name = $func->uploadName($_FILES["videofile"]["name"]);
				if($photo = $func->uploadImage("videofile", $config['photo']['man_photo'][$type]['file_type'], UPLOAD_FILE,$file_name))
				{
					$data['taptin'] = $photo;
		...
		else
		{
			...
			if($numberPhoto && $dataMultiTemp)
			{
				...
				for($i=0;$i<count($dataMultiTemp);$i++)
				{
					...
					if(isset($config['photo']['man_photo'][$type]['video_file']) && $config['photo']['man_photo'][$type]['video_file'] == true)
					{
						if(isset($_FILES["videofile".$i]))
						{
							$file_name = $func->uploadName($_FILES["videofile".$i]["name"]);
							if($photo = $func->uploadImage("videofile".$i, $config['photo']['man_photo'][$type]['file_type'], UPLOAD_FILE,$file_name.$i))
							{
								$success_photo = 1;
								$dataMulti['taptin'] = $photo;
							}
						}
					}
