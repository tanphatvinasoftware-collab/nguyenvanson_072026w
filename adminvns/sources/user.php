...
function save_item_admin()
{
	...
	if(isset($check_username['id']) && $check_username['id'] > 0) $func->transfer("Tên đăng nhập nay đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man_admin&p=".$curPage, false);
...
function save_item()
{
	...
	if(isset($check_username['id']) && $check_username['id'] > 0) $func->transfer("Tên đăng nhập nay đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man&p=".$curPage, false);

	if(isset($_FILES['file']))
	{
		$file_name = $func->uploadName($_FILES['file']["name"]);
		if($photo = $func->uploadImage("file", $config['user']['img_type'],UPLOAD_USER,$file_name))
		{
			$data['avatar'] = $photo;
			$row = $d->rawQueryOne("select id, avatar from #_member where id = ? limit 0,1",array($row['id']));
		...
		if(!empty($_POST))
		{
			...
			else
			{
				...
				if(isset($_FILES['file']))
				{
					...
					{
						$data['avatar'] = $photo;
						$row = $d->rawQueryOne("select id, avatar from #_user where id = ? limit 0,1",array($row['id']));
						if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_USER.$row['avatar']);
					}
				}
