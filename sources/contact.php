<?php 
	...
	if(isset($_POST['submit-contact']))
	{
        ...
        if(($scoreCaptcha >= 0.5 && $actionCaptcha == 'contact') || $testCaptcha == true)
		{
			...
			// Validate số điện thoại (dành cho số điện thoại 10 số của Việt Nam)
			$data['dienthoai'] = (isset($_POST['dienthoai']) && preg_match('/^0[0-9]{9}$/', $_POST['dienthoai'])) ? htmlspecialchars($_POST['dienthoai']) : '';
			if($data['dienthoai'] == '')
			{
				$func->transfer("Số điện thoại không hợp lệ",$config_base.'lien-he', false);
			}
			// Validate email bằng hàm có sẵn của PHP (khuyến khích dùng)
			$data['email'] = (isset($_POST['email']) && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) ? htmlspecialchars($_POST['email']) : '';
			if($data['email'] == '')
			{
				$func->transfer("Email không hợp lệ",$config_base.'lien-he', false);
			}
