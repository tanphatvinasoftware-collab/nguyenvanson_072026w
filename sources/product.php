...
if($id!='')
{
	...
	if(isset($_SESSION['daxem']) && $_SESSION['daxem'] != '') {
	...
	$levels = [$pro_list,$pro_cat,$pro_item,$pro_sub,$row_detail];
	foreach ($levels as $row) {
		if (!empty($row)) {
			$breadcr->setBreadCrumbs($row[$sluglang], $row['ten']);
		}
	}
...
else if($idi!='')
{
	...
	$levels = [$pro_list,$pro_cat,$pro_item];
	foreach ($levels as $row) {
		if (!empty($row)) {
			$breadcr->setBreadCrumbs($row[$sluglang], $row['ten']);
		}
...
else if($ids!='')
{
	...
	/* breadCrumbs */
	if(isset($title_crumb) && $title_crumb != '') $breadcr->setBreadCrumbs($com,$title_crumb);
	$levels = [$pro_list,$pro_cat,$pro_item,$pro_sub];
	foreach ($levels as $row) {
		if (!empty($row)) {
			$breadcr->setBreadCrumbs($row[$sluglang], $row['ten']);
		}
...
else if($idb!='')
{
	...
	/* breadCrumbs */
	if($pro_brand) $breadcr->setBreadCrumbs($pro_brand[$sluglang],$title_cat);
	$breadcrumbs = $breadcr->getBreadCrumbs();
}
else
{
